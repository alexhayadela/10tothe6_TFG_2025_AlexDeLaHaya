...
from db.sqlite.ingest_ohlcv import ingest_ohlcv,ingest_predictions
from db.sqlite.queries_ohlcv import _get_last_date as get_last_date_ohlcv, _get_last_date_predictions
from db.supabase.queries_news import _fetch_news_since
from db.supabase.queries_ohlcv import fetch_ohlcv_since, fetch_predictions_since
...
def predictions_migration(supabase: Client, conn: Connection) -> None:
    """Migrate predictions from supabase -> sqlite."""
    tickers = get_all_tickers()
    dfs = []
    for t in tickers:
        last_date = _get_last_date_predictions(conn, t)
        df = fetch_predictions_since(supabase, t, last_date)
        if not df.empty:
             df["date"] = pd.to_datetime(df["date"]).dt.strftime("%Y-%m-%d") # Sqlite compatible
             dfs.append(df)
    ...
    if dfs: 
         final_df = pd.concat(dfs, ignore_index=True)
         ingest_predictions(conn, final_df)
...
if __name__ == "__main__":
    ...
    with sqlite_connection() as conn:
        ...
        predictions_migration(supabase, conn)