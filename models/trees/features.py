...
def necessary_features(
    df: pd.DataFrame, ft_type: Literal["micro", "cross", "macro"] = "macro"
):
    keep_micro = [
        "log_ret_1",
        "log_ret_3",
...
def align_macro(df_macro: pd.DataFrame):
    ...
    full_index = pd.MultiIndex.from_product(
        [df["ticker"].unique(), idx_daily], names=["ticker", "date"]
    )
...
def ml_ready(
    ...
    # preprocess like ovlume > 0 (keep into account vix and ibex volume 0)
